...
public sealed class MetricStrip : Border
{
    ...
    [
        new("FPS", s => s.ShowFps, m => Number(m.Fps, "0"), 4),
        new("CPU", s => s.ShowCpuUsage, m => Number(m.CpuUsage, "0", "%"), 4),
        new("CPU TEMP", s => s.ShowCpuTemp, m => Number(m.CpuTemperature, "0", "°C"), 5),
        new("GPU", s => s.ShowGpuUsage, m => Number(m.GpuUsage, "0", "%"), 4),
        new("GPU TEMP", s => s.ShowGpuTemp, m => Number(m.GpuTemperature, "0", "°C"), 5),
        new("RAM", s => s.ShowRam, m => Pair(m.RamUsedGb, m.RamTotalGb), 11),
        new("VRAM", s => s.ShowVram, m => Pair(m.VramUsedGb, m.VramTotalGb), 9),
    ...
    public void Apply(AppSettings settings)
    {
        ...
        _characterWidth = new FormattedText("0", System.Globalization.CultureInfo.CurrentCulture,
            System.Windows.FlowDirection.LeftToRight, new Typeface(font, FontStyles.Normal, FontWeights.Normal, FontStretches.Normal),
            size, Brushes.White, VisualTreeHelper.GetDpi(this).PixelsPerDip).WidthIncludingTrailingWhitespace;
        Brush color;
        ...
        foreach (var metric in Definitions)
        {
            ...
            if (_values.Count > 0) _panel.Children.Add(new TextBlock { Text = "│", Foreground = Brushes.DimGray, FontSize = size, Margin = new Thickness(2, 0, 2, 0) });
            ...
            var value = new TextBlock { FontFamily = new FontFamily("Consolas"), FontSize = size, Foreground = color,
                Width = Math.Ceiling(_characterWidth * metric.Width), TextAlignment = TextAlignment.Right,
                TextTrimming = TextTrimming.None, VerticalAlignment = VerticalAlignment.Center };
    ...
    public void Update(MetricSnapshot snapshot)
    {
        ...
        foreach (var (text, metric) in _values)
        {
            ...
            // Reserve memory capacity rather than today's usage; widths never shrink
            // on sampling ticks. Right alignment leaves no empty tail at the corner.
            int characters = metric.Label switch
            {
                "RAM" when snapshot.RamTotalGb is double total => Pair(total, total).Length,
                "VRAM" when snapshot.VramTotalGb is double total => Pair(total, total).Length,
                _ => metric.Width
            };
            double needed = Math.Ceiling(_characterWidth * Math.Max(characters, value.Length));
            if (needed > text.Width) text.Width = needed;
